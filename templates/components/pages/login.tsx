...
import { Link, useNavigate } from 'react-router-dom'; // Замінив 'react-router' на 'react-router-dom'
...
export function Login() {
  ...
  const { login } = useAuth(); // Отримуємо функцію login
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const success = await login(username, password); // Передаємо username і password для входу
      if (success) {
        navigate('/tracker'); // Якщо вхід успішний, перенаправляємо на сторінку tracker
      } else {
