    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 md:py-12">
        ...
        <div className="flex items-start justify-between mb-8 md:mb-12">
          <div>
            <h1 className="text-3xl md:text-5xl font-bold mb-2">Tracker</h1>
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <div className="flex gap-2 bg-gray-100 p-1 rounded-full w-fit">
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${activeTab === 'expenses' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600'}`}
              >
                Expenses
              </button>
              <button
                onClick={() => setActiveTab('income')}
                className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${activeTab === 'income' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600'}`}
              >
                Income
              </button>
            </div>
            ...
            <div className="flex gap-2 text-sm flex-wrap">
              {['Day', 'Week', 'Month', 'Year'].map((period) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full transition-colors ${timePeriod === period.toLowerCase() ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                >
                  {period}
                </button>
              ))}
            ...
            <div className="bg-white rounded-xl p-3 border border-gray-200">
              <div className="flex items-center justify-between">
                <button onClick={() => handleDateChange('prev')} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors">←</button>
                <button
                ...
                <button onClick={() => handleDateChange('next')} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors">→</button>
              </div>
              {showDatePicker && (
            ...
            {/* Мобільний перемикач діаграми */}
            <button
              className="md:hidden w-full py-2.5 rounded-full border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition-colors"
              onClick={() => setShowChart(!showChart)}
            >
              {showChart ? 'Сховати діаграму ▲' : 'Показати діаграму ▼'}
            </button>

            {/* Діаграма на мобільному */}
            {showChart && (
                  ...
                  {filteredData.length > 0 ? (
                    <>
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={filteredData} cx="50%" cy="50%" innerRadius={80} outerRadius={130} paddingAngle={2} dataKey="amount" minAngle={5}>
            ...
            <div className="space-y-3">
              ...
              <div className="max-h-96 overflow-y-auto pr-2">
                {filteredData.map((item) => (
                  <div key={item.id} className="flex items-center justify-between py-3 border-b border-gray-200 group">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => handleDelete(item.id)} className="opacity-0 group-hover:opacity-100 transition-opacity text-red-500 hover:text-red-700">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
                ...
                {filteredData.length === 0 && (
                  <div className="text-center py-8 text-gray-500">No transactions for this period</div>
                )}
              </div>
            </div>
          </div>

          {/* Right: Chart — тільки десктоп */}
            ...
            <div className="relative w-96 h-96">
              {filteredData.length > 0 ? (
                <>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={filteredData} cx="50%" cy="50%" innerRadius={110} outerRadius={170} paddingAngle={2} dataKey="amount" minAngle={5}>
                        {filteredData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry.color} />
                        ))}
                      </Pie>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-md">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold">Add {activeTab === 'expenses' ? 'Expense' : 'Income'}</h2>
            ...
            <form onSubmit={handleAddItem} className="space-y-4">
              <div>
                ...
                <input type="text" value={newItemName} onChange={(e) => setNewItemName(e.target.value)} placeholder={`Enter ${activeTab === 'expenses' ? 'expense' : 'income'} name`} className="w-full px-5 py-3.5 rounded-full border-2 border-gray-200 focus:outline-none focus:border-purple-300 bg-white" required />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" value={newItemDate} onChange={(e) => setNewItemDate(e.target.value)} className="w-full px-5 py-3.5 rounded-full border-2 border-gray-200 focus:outline-none focus:border-purple-300 bg-white" required />
              </div>
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 py-3.5 rounded-full bg-purple-300 text-gray-900 font-medium hover:bg-purple-400 transition-colors">Add</button>
              </div>
            </form>
          </div>
        </div>
